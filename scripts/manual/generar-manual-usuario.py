...
nota('Mientras bajan, la caja dice con qué se está actualizando —«Actualizando la caja con clientes»— y una barra de '
     'progreso muestra cuánto lleva, así que se ve de un vistazo si avanza o se quedó parado.')
...
tabla(['Situación', 'Qué pasa y qué hacer'],
      [['La caja o el Central avisan que falta la base de datos', 'No se ejecutó el script de creación en ese equipo, o el sistema está apuntando a otra base. Ejecute scripts/base-datos/estructura_base_datos_central.sql en el servidor o scripts/base-datos/estructura_base_datos_pos.sql en la caja.'],
       ...
       ['A la caja le faltan artículos o clientes que en el Central sí están', 'La caja solo pide lo que cambió en el Central, así que lo que se haya perdido en su base no vuelve por su cuenta: allá no pasó nada. En la pantalla de ingreso, «Configuración de la caja» → «Volver a bajar todo del Central», con el usuario del Central que lo autorice. La caja baja el catálogo entero otra vez; tarda y mientras dura no se puede vender. Lo que ya tiene no se duplica: se actualiza.'],
