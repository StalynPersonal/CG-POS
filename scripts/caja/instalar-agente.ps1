...
<#
.SYNOPSIS
    Instala CG-POS Agente como servicio de Windows en una caja.
.DESCRIPTION
    - Crea el servicio "CgPosAgente" con inicio automático retrasado.
    - Lo hace depender de la instancia de SQL Server Express local.
    - Configura reinicio automático ante fallos.
.EXAMPLE
    .\instalar-agente.ps1 -RutaEjecutable "C:\CGPOS\Agente\CgPos.Pos.Agente.exe"
#>
param(
    [string] $RutaEjecutable = "C:\CGPOS\Agente\CgPos.Pos.Agente.exe",
...
if (Get-Service -Name $nombre -ErrorAction SilentlyContinue) { throw "El servicio $nombre ya está instalado. Use desinstalar-agente.ps1 primero." }
...
sc.exe create $nombre binPath= "`"$RutaEjecutable`"" start= delayed-auto depend= $ServicioSql DisplayName= "CG-POS Agente" | Out-Null
...
Write-Host "Verifique: http://localhost:5180/salud"
