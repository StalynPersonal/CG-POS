...
public static class RutasApiSeguridad
{
    public static IEndpointRouteBuilder MapearApiSeguridad(this IEndpointRouteBuilder aplicacion)
    {
        ...
        sesion.MapPost("/ingreso", (SolicitudIngreso solicitud, IServicioAutenticacion autenticacion, EmisorTokens emisor, IParametros parametros, CancellationToken cancelacion) =>
            IngresarAsync(new CredencialUsuario(solicitud.CodigoUsuario ?? string.Empty, solicitud.Clave ?? string.Empty), autenticacion, emisor, parametros, cancelacion));
        ...
        CancellationToken cancelacion)
    {
        ...
        var mensaje = MensajesSeguridad.Para(resultado.Motivo!.Value, resultado.BloqueadoHasta);
    ...
    private static async Task<IResult> AutorizarAsync(SolicitudAutorizacion solicitud, ClaimsPrincipal usuario, IServicioAutorizacion servicio, CancellationToken cancelacion)
    {
        ...
        var credencial = new CredencialUsuario(solicitud.CodigoSupervisor ?? string.Empty, solicitud.Clave ?? string.Empty);
