...
public class OpcionesJsonPruebas
{
    private sealed record Ejemplo(string Numero, decimal Total, EstadoMensajeSalida Estado, string? Comentario = null);

    [Fact]
    public void Serializa_en_camelCase_con_enums_como_texto_y_sin_nulos()
    {
        var json = JsonSerializer.Serialize(new Ejemplo("E320000000001", 850.00m, EstadoMensajeSalida.Pendiente), OpcionesJson.Predeterminadas);
    ...
    {
        var original = new Ejemplo("E320000000001", 2175.34m, EstadoMensajeSalida.Confirmado, "Prueba");
