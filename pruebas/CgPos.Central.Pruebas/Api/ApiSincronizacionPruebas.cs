...
public class ApiSincronizacionPruebas(CentralEnPruebas central)
{
    ...
    public async Task El_cliente_de_la_caja_se_autentica_y_sincroniza_contra_el_central_real()
    {
        ...
        var caja = new ClienteCentralHttp(http, "01", "01", new CredencialCajaEnMemoria(secreto), TimeProvider.System);
        var mensaje = MensajeVenta(CentralEnPruebas.CajaUno, NuevoEncf());
        ...
        var conOtroSecreto = await new ClienteCentralHttp(http, "01", "01", new CredencialCajaEnMemoria(secreto + "x"), TimeProvider.System).EnviarAsync(MensajeVenta(CentralEnPruebas.CajaUno, NuevoEncf()));
