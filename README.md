...
- **API de artículos y precios:** `GET /api/maestros/articulos?buscar=&pagina=&tamano=` y `PUT /api/maestros/articulos/{id}`; `GET /api/precios/articulos?buscar=&pagina=&tamano=` y `PUT /api/precios/articulos/{id}`; `GET /api/precios/familias`; `GET /api/precios/topes` y `PUT /api/precios/topes/{id}`.
