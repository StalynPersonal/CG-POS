...
- **Facturas de cualquier tienda:** si la factura no es de esta caja se le pide al Central (`GET /api/facturas/{numero}`, por número o por e-NCF) y se guarda en la tabla temporal `FacturasConsultadas`. El Central manda lo **ya devuelto de cada línea en toda la empresa**, que es lo único que la caja no puede saber sola, y le **retiene esas líneas** mientras emite (`POST /api/facturas/{numero}/reservas`, con `Central.Devoluciones.MinutosReserva`), para que dos tiendas no devuelvan la misma mercancía. La nota sale siempre con el **certificado y el rango de e-NCF de la caja que la emite**, no de la que vendió, y la copia de la factura **se borra al emitirla**: en la caja no queda guardada la venta de otra tienda. Sin conexión al Central solo se devuelven las facturas de esta caja, y la pantalla lo dice así.
...
- **API:** `GET /api/devoluciones/factura/{numero}` (busca en la caja y, si no es suya, en el Central), `POST /api/devoluciones`, `GET /api/devoluciones/notas-credito/{codigo}`, `POST /api/devoluciones/{id}/reimprimir`, `GET /api/devoluciones/motivos`.
