...
| `Fiscal.PorcentajeRetencionLey3223` | Retención de la Ley 32-23 en facturas de régimen especial (E44) | No (sin configurar, no se retiene) |
...
- **Central:** `Central:Url` para el Central real (`POST api/sincronizacion/mensajes`), con `Caja:Sucursal` y `Caja:Codigo` (los códigos de su sucursal y de la caja, de 1 a 99). La credencial no se configura: la caja la pide al Central y alguien la acepta allá (ver *Enrolamiento*). También `Central:Modo = Simulado`, que guarda los mensajes en `Central:CarpetaSimulada` con la misma idempotencia y validación de hash. Sin ninguno, la barra muestra «Sin Central».
...
- **Enrolamiento:** la caja recién instalada no trae secreto. Se anuncia al Central con la huella de su equipo y queda **pendiente** hasta que alguien la acepta en *Organización → Solicitudes de cajas*; recién entonces se le entrega su credencial, que ella guarda cifrada con DPAPI en su propio equipo (`Central:ArchivoCredencial`, por defecto `C:\CGPOS\credencial-caja.dat`). Al aceptarla, la credencial queda **atada a ese equipo**: presentarla desde otra máquina se rechaza y queda auditado, que es como se detecta una configuración copiada. Para mudar la caja a otro equipo se usa **Liberar equipo** (con motivo y auditado) y el equipo nuevo vuelve a pedir. Las credenciales emitidas a mano antes de esto no tienen equipo: se atan al primero que las use.
...
- **API:** `POST /api/sesion/ingreso`, `POST /api/sesion/renovar`, `GET /api/sesion/actual`, `POST /api/sesion/cerrar`, `POST /api/sesion/contrasena`; `POST /api/cajas/{id}/credencial`, `POST /api/cajas/{id}/credencial/revocar` y `POST /api/cajas/{id}/credencial/liberar-equipo`, más `GET /api/enrolamiento` y `POST /api/enrolamiento/{id}/aprobar|rechazar` (permiso `Central.Dispositivos.Administrar`); `POST /api/dispositivos/solicitud`, `POST /api/dispositivos/token` y `GET /api/dispositivos/actual` para las cajas.
...
- *Credenciales de las cajas* (permiso `Central.Dispositivos.Administrar`): en **Solicitudes de cajas** se aceptan o rechazan las cajas que piden entrar, viendo el equipo, su dirección y la hora del pedido. Desde **Cajas** se ve a qué equipo está atada cada credencial y se puede **liberar el equipo**, revocarla o emitir una nueva a mano.
...
1. **Central:** instalar, cargar empresa, sucursales, cajas, parámetros y maestros. Las credenciales de las cajas no se emiten por adelantado: cada caja las pide al instalarse y se aceptan en Solicitudes de cajas.
